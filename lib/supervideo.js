...
(function( $ ) {
  ...
  var methods = {
    ...
    on: function( ) {
      ...
      $wrapper_div.css("z-index", "-9999");
      $wrapper_div.css("width", '100%');
      $wrapper_div.css("height", '100%');
      $wrapper_div.css("top", '0');
      $wrapper_div.css("left", '0');

      $wrapper_div.append("<button id='supervideo_next'>Next</div>");
      $wrapper_div.append("<button id='supervideo_prev'>Prev</div>");

      $('#supervideo_next').bind('click', slide_next);
      $('#supervideo_prev').bind('click', slide_prev);
      ...
      if (_v.length > 0) _v[0].data('supervideo').$vdiv.css("display", "block");
    ...
    off: function( ){
      ...
      return this.each(function(){
        ...
        filter_allowed_elements (this, function(elem)
        {
          ...
          $('#supervideo').remove();
          $('#supervideo_prev').remove();
          $('#supervideo_next').remove();
  ...
  function slide_next( ){
    if (_v.length > 1){
      ...
      if(_v[curr].is("video")) _v[curr].get(0).pause();
      curr = next;
      ...
      $(div_old).animate({ "left": "-=" + width + "px" }, "slow", function(){
        $(this).css("display", "none");
      });

      $(div_new).animate({"left": "-=" + width + "px"}, "slow");
    }
  }
  ...
  function resize( elem )
  {
    return function(){
      ...
      $elem.css("top", ( wh - $(elem).height() ) / 2+$(window).scrollTop() + "px" );
      $elem.css("left", ( ww - $(elem).width() ) / 2+$(window).scrollLeft() + "px" );
    }
  }
